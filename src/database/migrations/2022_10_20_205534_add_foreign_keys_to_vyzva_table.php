...
class AddForeignKeysToVyzvaTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('vyzva', function (Blueprint $table) {
            $table->foreign(['typ_vyzvy_idtyp_vyzvy'], 'fk_vyzva_typ_vyzvy')->references(['idtyp_vyzvy'])->on('typ_vyzvy')->onUpdate('NO ACTION')->onDelete('NO ACTION');
    ...
    {
        Schema::table('vyzva', function (Blueprint $table) {
            $table->dropForeign('fk_vyzva_typ_vyzvy');
